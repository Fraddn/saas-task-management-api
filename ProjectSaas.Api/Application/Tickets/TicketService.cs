...
public sealed class TicketService : ITicketService
{
    ...
    public async Task<TicketDto> UpdateAsync(Guid ticketId, UpdateTicketRequest request, CancellationToken ct)
    {
        ...
        // Optimistic concurrency check (manual RowVersion)
    ...
    {
        // Defence in depth: enforce Admin role in service layer
        ...
        // Validate assignee belongs to same tenant (avoid cross-tenant assignment)
        // Assumes you have _db.Users DbSet<User> and User has OrganisationId.
        ...
        // Optimistic concurrency check (manual RowVersion) - same style as UpdateAsync
        if (ticket.RowVersion != request.RowVersion)
            throw new ConcurrencyConflictException("Concurrency conflict. Please refresh and retry.");
    ...
    public async Task SoftDeleteAsync(Guid ticketId, int rowVersion, CancellationToken ct)
    {
        ...
        // Optimistic concurrency check (manual RowVersion) - same style as UpdateAsync/AssignAsync
